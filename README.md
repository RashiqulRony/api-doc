...
<h2>Laravel API Doc - The API Documentation Generator</h2>
...
Laravel api-doc generator (interactive documentation generator) is a seamless and complete plugin for generating API documentation from your Laravel's codebase. It is inspired by the laravel-apidoc-generator, ReDoc and the Open API initiative from Swagger. api-doc has been built with extendability so that it can easily adapt with your use case.
...
```sh
$ composer require rashiqulrony/api-doc
```
...
```bash
php artisan vendor:publish --tag=api-doc-config
```
...
$app->register(\RashiqulRony\ApiDoc\IDocLumenServiceProvider::class);
```
- Copy the config file from `vendor/rashiqulrony/apidoc/config/apidoc.php` to your project as `config/api-doc.php`. Then add to your `bootstrap/app.php`:
```php
$app->configure('api-doc');
```
...
'servers' => [
    [
        'url' => 'https://www.rashron.com',
        'description' => 'App live server.',
    ],
    [
        'url' => 'https://test.rashron.com',
...
```php
'external_description' => 'ApiDoc.info', // Route name for external description, leave empty to use default description
```
...
This is where you can set languages used to write request samples. Each item in array is used to generate a request template for a given language. New languages can be added and the existing ones modified after. You can add or edit new languages tabs by publishing the view files and editing them or adding custom view files to:
 ```php
'resources/views/vendor/ApiDoc/languages/LANGUAGE.blade.php',
 ```
...
The `apidoc:custom` command allows you to generate API documentation using a custom configuration file. This is useful when you need to generate documentation with different settings without modifying the default configuration.
...
```sh
php artisan apidoc:custom {config?}
```
...
1. **Create a Custom Configuration File:**
   ...
   Create a custom configuration file in the `config` directory. The file should follow the naming convention `apidoc.{config}.php`, where `{config}` is the name you will use when running the command.

   Example for `config/apidoc.ecommerce.php`:
   ```
   // config/apidoc.ecommerce.php
   return [
...
2. **Run the Command:**
   ...
   Example:
   ```
   php artisan apidoc:custom ecommerce
   ```
...
1. **Create Configuration Files:**
   ...
   - `config/apidoc.ecommerce.php`
   - `config/apidoc.user.php`

2. **Run the Command for Each API:**

   ```
   php artisan apidoc:custom ecommerce
   php artisan apidoc:custom user
   ```
...
Example for `apidoc.ecommerce.php` configuration:
...
Route::group([], function () {
    // Set the apidoc config to the ecommerce config
    config(['apidoc' => config('apidoc.ecommerce')]);

    // Define the route for the user documentation
    Route::view(config('apidoc.path'), 'apidoc::documentation');
});
```
